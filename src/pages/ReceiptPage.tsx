    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-900 flex flex-col items-center pt-24 pb-12 px-4">
      <div className="w-full max-w-4xl">
        ...
        <div className="space-y-6">
          <div className="flex justify-between">
            <span className="font-semibold text-gray-600 dark:text-gray-300">Receipt ID:</span>
            <span className="font-mono text-gray-800 dark:text-gray-100">{enrollment.receiptId}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-semibold text-gray-600 dark:text-gray-300">Course Name:</span>
            <span className="text-lg font-bold text-primary-600 dark:text-primary-400">{enrollment.courseName}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-semibold text-gray-600 dark:text-gray-300">Original Price:</span>
            <span className="text-gray-800 dark:text-gray-200">₹{enrollment.originalPrice || enrollment.price}</span>
          </div>
          {enrollment.discountApplied > 0 && (
            <>
              <div className="flex justify-between">
                <span className="font-semibold text-gray-600 dark:text-gray-300">Discount Applied:</span>
                <span className="text-green-600 dark:text-green-400">-₹{enrollment.discountApplied}</span>
              </div>
          ...
          {enrollment.referralCode && (
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-dark-700">
              <div className="flex justify-between">
                <span className="font-semibold text-gray-600 dark:text-gray-300">Referred by Teacher ID:</span>
                <span className="text-blue-600 dark:text-blue-400 font-mono">{enrollment.referralCode}</span>
              </div>
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                Thank you for using a referral code! Your teacher will receive 10 points for this enrollment.
              </p>
            </div>
          )}
          <div className="flex justify-between">
            <span className="font-semibold text-gray-600 dark:text-gray-300">Enrollment Date:</span>
            <span className="text-gray-800 dark:text-gray-100">{new Date(enrollment.enrollmentDate).toLocaleDateString()}</span>
          </div>
          <div className="flex justify-between">
            <span className="font-semibold text-gray-600 dark:text-gray-300">Status:</span>
            <span className="px-3 py-1 text-sm font-semibold text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">{enrollment.status}</span>
          </div>
        </div>
        <div className="mt-10 pt-6 border-t border-gray-200 dark:border-dark-700 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">This is an official receipt for your records. If you have any questions, please contact our support team.</p>
        </div>
      </div>
